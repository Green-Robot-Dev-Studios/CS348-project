...
export const pickWinnerFood = async (context: HookContext) => {
    ...
    if (context.data.approved) {
        ...
        const approvedCount = await db.raw(
            ...
            FROM votes WHERE foodId = ? AND roomId = ?;`,
    ...
    const votedFoodCount = await db.raw(
        ...
        FROM (
            ...
            FROM votes v RIGHT OUTER JOIN connections c ON v.userId = c.userId and v.roomId = c.roomId
            WHERE c.roomId = ? GROUP BY c.userId
    ...
    if (votedFoodCount[0][0].min === closeFoodCount) {
        ...
        const maxVote = await db.raw(
            ...
            WITH countedVotes AS (
                ...
                FROM votes v WHERE v.roomId = ? GROUP BY v.foodId
            ), MaxVotes AS (
                SELECT MAX(cv.foodVotes) as maxVotes FROM countedVotes cv
            )
            SELECT cv.foodId
